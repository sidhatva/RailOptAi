...
export const CorridorTimeline = () => {
  const { selectedCorridor, trains, blockPlans } = useRailway();
      ...
      <div className="w-full overflow-x-auto overscroll-x-contain">
        ...
        <div className="space-y-4 pt-2">
          {corridorTracks.map((track) => {
            const trackOccurrences = timelineOccurrences.filter((item) => {
                  ...
                  {trackOccurrences.map((occ) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseEnter={() => setHoveredEntity(occ)}
                        onMouseLeave={() => setHoveredEntity(null)}
                        className={`absolute top-2 bottom-2 rounded-md bg-gradient-to-r ${occ.color} ${occ.borderColor} border shadow-sm flex items-center px-2 text-[10px] font-mono font-bold ${occ.textColor} cursor-pointer transition-all hover:scale-102 hover:z-20 truncate`}
                        style={{
                          left: `${safeStart}%`,
                          width: `${widthP}%`
                        }}
                      >
                        <TrainIcon className="w-3 h-3 mr-1 flex-shrink-0 opacity-80" />
                        <span className="truncate">{occ.trainNo} {occ.name}</span>
                        {occ.delayMins > 0 && (
                          <span className="ml-1 px-1 rounded bg-red-950/80 text-red-200 text-[9px] flex-shrink-0">
                            +{occ.delayMins}m
                          </span>
                        )}
                      </div>
          ...
          <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-mono text-slate-600 dark:text-slate-400">
            {hoveredEntity.isBlock ? (
              ...
              <>
                ...
                <div>
                  <span className="text-slate-400">Route Clearance:</span>{' '}
                  <strong className="text-slate-800 dark:text-slate-200">Automatic Signal Green</strong>
                </div>
                <div>
                  <span className="text-slate-400">Section Priority:</span>{' '}
                  <strong className="text-amber-500">Priority 1 (VIP)</strong>
                </div>
              </>
            )}
          </div>
